...
describe("reduce", () => {
  ...
  test("Sum object keys and values together", () => {
    ...
    expect(
      reduce(obj, (result, value, key) => result + value + key, 0)
    ).toEqual(40);
  });
  ...
  test("Sum with empty object", () => {
    expect(reduce({}, (result, value, key) => result + value + key, 0)).toEqual(
      0
    );
  });
  test("Leave collection undefined", () => {
    ...
    expect(
      reduce(undefined, (result, value, key) => result + value + key, 0)
    ).toEqual(0);
  });
  ...
  test("Leave accumulator undefined with sum function", () => {
    ...
    expect(reduce(array, (result, value) => result + value, undefined)).toEqual(
      NaN
    );
  });
});
